...
% CTRL + T
	...
	Ebben a fejezetben fogjuk előkészíteni a Csebisev-tétel bizonyításához szükséges állításokat. Ezek többnyire az $n!$ vagy a binomiális együtthatók számelméletével kapcsolatosak lesznek, azokat próbálják becsülni, vagy prímtényezős alakjukat leírni.
	...
	\begin{proof}
		...
		Nyilvánvalóan mivel nem az első sorról van szó, így a középső elemen kívül vannak más elemek is, tehát a vizsgált elem kisebb, mint a teljes sor összege, ami $2^{2n} = (2^2)^n = 4^n$. Ezzel a bizonyítandó jobboldalát meg is kaptuk: ${2n\choose n} < 4^n$.
	...
	\begin{pl}
		...
		A $7$ már a $7$ és a $14$ miatt is benne van. Általánosan annyi számban szerepel egy adott prím, ahány az adott prímmel osztható szám van $15$-ig, tehát $\left \lfloor \dfrac{15}{7} \right \rfloor = 2$.
	...
	Nézzük meg mit mondhatunk általánosan!
	...
	\begin{proof}
		...
		Az első hasznos észrevételünk, hogy elegendő $x\geq 2$-vel foglalkoznunk, különben a baloldali szorzat $0$ (hiszen 2 alatti pozitív prímszám nincs), a $4^x$ exponenciális függvényre pedig valóban alsó korlát a $0$. Látható, hogy $x=2$ és $x=3$ esetén is igaz:
	...
	\begin{theo}\label{otodik}
		Amennyiben $n\geq 1$ akkor ${2n\choose n} = \prod p^{\alpha}$ esetén $p^{\alpha} \leq 2n$.
	\end{theo}

	\begin{proof}
		Írjuk fel definíció szerint a binomiális együtthatót:
		\[ {2n\choose n} = \dfrac{(2n)!}{(n!)!}  \]
		...
		igaz, hogy itt a második esetében elegendő lenne $n$-ig menni a szumma alatti feltételben, de korábban (az említett tétel bizonyításakor) megbeszéltük, hogy ha tovább is megyünk az sem gond, mert csak $0$-kat adunk össze. Csupán azért választjuk mst ezt a feltételt, mert így láthatóan összevonhatóak a szummák:
		...
		\[  \left \lfloor \dfrac{2n}{p^i} \right \rfloor - 2\cdot  \left \lfloor \dfrac{n}{p^i} \right \rfloor < \dfrac{2n}{p^i} - 2\cdot \left( \dfrac{n}{p^i} \right)  =  2  \]
		De ha a szumma minden egyes tagja kisebb $2$-nél, akkor csak $0$-kat vagy $1$-eseket, pontosabban mivel előbbi lényegtelen csak $1$-eseket adunk össze amíg $p^i\leq 2n$, tehát megszámoljuk $p$-nek $1$-től hanyadik hatványáig lesz igaz, hogy $p^i\leq 2n$. Ez pedig azzal ekvivalens, hogy mi az a legnagyobb hatványa $p$-nek melyre $p^i\leq 2n$.
	...
	\begin{proof}
		...
		\[ {2n\choose n} = \prod p^\alpha = \prod_{p\leq \sqrt{2n}} p^\alpha \cdot \prod_{\sqrt{2n}< p \leq \frac{2}{3}n} p^\alpha \cdot \prod_{\frac{2}{3}n<p\leq n} p^\alpha \cdot \prod_{n<p\leq 2n} p^\alpha \]
		...
		és mivel $p\leq \sqrt{2n}$, így ez egy legfeljebb $\sqrt{2n}$ tényezős szorzat, még ha $1$-től $2n$-ig az összes számot prímnek veszem akkor is, tehát tovább becsülve
		...
		Ezt most felülről becsüljük úgy, hogy nem csak a $\sqrt{2n}$ és $\frac{2}{3}n$ közötti prímhatványokat szorozzuk össze, hanem $\frac{2}{3}n$-ig az összeset:
		...
		\[ {2n\choose n} = \dfrac{(2n)!}{n!\cdot n!} = \dfrac{1\cdot 2\cdot \ldots \cdot \frac{2}{3}n \cdot \ldots \cdot \color{red}p\color{black} \cdot \ldots \cdot n \cdot (n+1) \cdot \ldots \color{red}2p\color{black} \cdot \ldots \cdot 2n}{(1\cdot \ldots \cdot \color{red}p\color{black} \cdot \ldots \cdot n) \cdot (1\cdot \ldots \cdot \color{red}p\color{black} \cdot \ldots \cdot n)}  \]
		...
		\[ {2n\choose n} \leq (2n)^{\sqrt{2n}} \cdot 4^{\frac{2}{3}n} \cdot 1 \cdot \prod_{n<p\leq 2n} p   \]
	...
	Mivel ${2n\choose n}$ a középső tag továbbra is a sorából, ezért az átlagnál nagyobb. De az előző, előadáson elhangzó bizonyításon erre úgy tekintettünk, hogy $2n+1$ tag van a sorában. Valójában ebből a szélső kettő nagyon pici, csupán $1$-ek mindig, vonjuk őket össze $1$ taggá. A sorösszeg ekkor továbbra is $4^n$, viszont az átlagolandó elemek száma $2n$, így már ki is jött a finomabb állítás. %${2n\choose n} > \frac{4^n}{2n}$.
	...
	Bernoulli-egyenlőtlenség: Amennyiben $h\geq -1$ akkor $(1+h)^n \leq 1+nh$ és egyenlőség csak $n=0$ vagy $n=1$ vagy $h=0$ esetén áll fenn.
	...
	Fontos megállapítás az is, hogy az egészrész definíciója szerint $x - 1 < \color{blue} \lfloor x \rfloor \leq x \color{black}$ azaz $\color{red} x<\lfloor x \rfloor + 1 \color{black}$.
	...
	\end{equation}
	Mivel $n\leq 50$-re láttuk az állítást eddig is, ezért feltehető, hogy $n\geq 50$, azaz
